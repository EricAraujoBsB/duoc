...
export function PrimaryServiceCard({service}: CardProps) {
    return (
        <article style={{ height: `${280}px`, width: `${175}px` }} className=" bg-[#0B0E10] text-black rounded-xl border border-[#424242] relative overflow-hidden">
            <div className="relative w-[175px] h-[112.5px]">
                ...
                <h1 className="absolute bottom-0 text-white font-bold ml-2.5 text-[13px] z-30">{service.nome}</h1>
            </div>
            <section className="px-2.5 mt-4 p">
                <p className="text-[9px] text-gray-400 line-clamp-3">{service.descricao}</p>
                <ul className="text-[9px] text-gray-400 list-disc ml-5 mt-3 font-bold marker:text-amber-400 ">
    ...
    return (
        <article style={{ height: `${420}px`, width: `${300}px` }} className=" bg-[#0B0E10] text-black rounded-xl border border-[#424242] overflow-hidden">
            ...
            <section className="px-5 mt-5">
                ...
                <Link href={`/service/${service.id}`} className="text-[13px] text-amber-400 cursor-pointer flex mt-4 gap-1 transition hover:scale-105">
                    Saiba Mais
